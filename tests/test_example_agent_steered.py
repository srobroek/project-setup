"""Thin wrapper: runs the agent-steered example module tests.

The authoritative test file lives at:
  skills/project-setup/examples/agent-steered/test_agent_steered.py
...
_EXAMPLE_TEST = (
    ...
    / "examples"
    / "agent-steered"
    / "test_agent_steered.py"
)

_spec = importlib.util.spec_from_file_location("test_agent_steered", _EXAMPLE_TEST)
assert _spec and _spec.loader, f"cannot locate example test at {_EXAMPLE_TEST}"
_mod = importlib.util.module_from_spec(_spec)
sys.modules["test_agent_steered"] = _mod
